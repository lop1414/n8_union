...
CREATE TABLE `product_weixin_mini_programs` (
  ...
  `guide_url` varchar(255) NOT NULL COMMENT '引导页url',
  `guide_path` varchar(255) NOT NULL COMMENT '小程序引导路径',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `product_id` (`product_id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='产品-小程序引导关注配置';
